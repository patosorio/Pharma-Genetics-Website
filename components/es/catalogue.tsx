    ...
    <section id="catalogue" className="py-16 bg-fenopharm-dark">
      <div className="container mx-auto px-4">
        <div className="text-center mb-12">
          <h2 className="text-3xl md:text-4xl font-light text-fenopharm-light mb-4 tracking-tight">CATÁLOGO 2025</h2>
        </div>

        {isLoading ? (
          <div className="text-center py-12">
            <p className="text-fenopharm-light">Cargando catálogo...</p>
          </div>
        ) : catalogueData.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-fenopharm-light">No hay genética disponible en este momento.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {displayedItems.map((genetics) => (
              <GeneticsCardEs key={genetics.id} genetics={genetics} />
            ))}
          </div>
        )}
        ...
        <div className="text-center space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-6">
            ...
            >
              ...
              {isDownloading ? "Descargando..." : "Descargar Catálogo"}
            </Button>
          </div>
        </div>
      </div>
...
function GeneticsCardEs({ genetics }: { genetics: GeneticsData }) {
  return (
      ...
      <CardContent className="p-6">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between items-center">
              <span className="text-xs text-fenopharm-gray">Tipo:</span>
              <span className="text-xs text-fenopharm-dark">{genetics.genotype}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs text-fenopharm-gray">Floración:</span>
              <span className="text-xs text-fenopharm-dark">{genetics.time}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-xs text-fenopharm-gray">Rendimiento:</span>
          ...
          <div className="pt-2 border-t border-fenopharm-light">
            <p className="text-xs text-fenopharm-gray font-light">
              Terpenos Principales: {[genetics.main_terpens, genetics.terpene_1, genetics.terpene_2, genetics.terpene_3].filter(Boolean).join(', ')}
            </p>
          </div>
        </div>
      </CardContent>
